  ...
  inConfig(Compile)(Transform.configSettings) ++
    inConfig(Compile)(
      Transform.transSourceSettings ++ Seq(
        ...
        Transform.sourceProperties := Map("cross.package0" -> "xsbt",
                                          "cross.package1" -> "boot")
      ))

// The interface JAR for projects which want to be launched by sbt.
lazy val launchInterfaceSub =
  minProject(file("launcher-interface"), "Launcher Interface")
    .settings(javaOnly)
    .settings(
      ...
      mimaPreviousArtifacts := Set(
        organization.value % moduleName.value % "1.0.1")
    )
...
lazy val launchSub = noPublish(
  baseProject(file("launcher-implementation"), "Launcher Implementation"))

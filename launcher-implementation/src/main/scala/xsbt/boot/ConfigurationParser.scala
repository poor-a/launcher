...
object ConfigurationParser {
  ...
  def ids(value: String) =
    trim(substituteVariables(value).split(",")).filter(isNonEmpty)

  private[this] lazy val VarPattern =
    Pattern.compile("""\$\{([\w.]+)(\-(.*))?\}""")
...
class ConfigurationParser {
  ...
  def apply(reader: Reader): LaunchConfiguration =
    Using(new BufferedReader(reader))(apply)
  private def apply(in: BufferedReader): LaunchConfiguration =
    processSections(processLines(readLine(in, Nil, 0)))
  private final def readLine(in: BufferedReader,
                             accum: List[Line],
                             index: Int): List[Line] =
    in.readLine match {
  ...
  private def newReader(file: File) =
  ...
  private def readRepositoriesConfig(
      in: BufferedReader): List[Repository.Repository] =
    processRepositoriesConfig(processLines(readLine(in, Nil, 0)))
  def processRepositoriesConfig(
      sections: SectionMap): List[Repository.Repository] =
  ...
  def processSections(sections: SectionMap): LaunchConfiguration = {
    val ((scalaVersion, scalaClassifiers), m1) =
      processSection(sections, "scala", getScala)
    val ((app, appClassifiers), m2) = processSection(m1, "app", getApplication)
    val (defaultRepositories, m3) =
      processSection(m2, "repositories", getRepositories)
    val (boot, m4) = processSection(m3, "boot", getBoot)
    val (logging, m5) = processSection(m4, "log", getLogging)
    val (properties, m6) =
      processSection(m5, "app-properties", getAppProperties)
    val ((ivyHome, checksums, isOverrideRepos, rConfigFile), m7) =
      processSection(m6, "ivy", getIvy)
    ...
    val ivyOptions =
      IvyOptions(ivyHome, classifiers, repositories, checksums, isOverrideRepos)

    // TODO - Read server properties...
    new LaunchConfiguration(scalaVersion,
                            ivyOptions,
                            app,
                            boot,
                            logging,
  ...
  def getClassifiers(m: LabelMap,
                     label: String): (Value[List[String]], LabelMap) =
    process(m, "classifiers", processClassifiers(label))
  def processClassifiers(label: String)(
      value: Option[String]): Value[List[String]] =
    value.map(readValue[List[String]](label)) getOrElse new Explicit(Nil)

  def getVersion(m: LabelMap,
                 label: String,
                 defaultName: String): (Value[String], LabelMap) =
    process(m, "version", processVersion(label, defaultName))
  def processVersion(label: String, defaultName: String)(
      value: Option[String]): Value[String] =
    value
      .map(readValue[String](label))
      .getOrElse(new Implicit(defaultName, None))

  def readValue[T](label: String)(
      implicit read: String => T): String => Value[T] = value0 => {
    val value = substituteVariables(value0)
    if (isEmpty(value))
      Pre.error(
        label + " cannot be empty (omit declaration to use the default)")
    try { parsePropertyValue(label, value)(Value.readImplied[T]) } catch {
      case e: BootException => new Explicit(read(value))
    }
  }
  ...
  def process[K, V, T](sections: ListMap[K, V],
                       name: K,
                       f: V => T): (T, ListMap[K, V]) =
    (f(sections(name)), sections - name)
  def check(map: ListMap[String, _], label: String): Unit =
    if (map.isEmpty) ()
    else Pre.error(map.keys.mkString("Invalid " + label + "(s): ", ",", ""))
  def check[T](label: String, pair: (T, ListMap[String, _])): T = {
  ...
  def getOrNone[K, V](map: ListMap[K, Option[V]], k: K) =
    orElse(map.get(k), None)
  ...
  def bool(map: LabelMap,
           name: String,
           default: Boolean): (Boolean, LabelMap) = {
  ...
  def getIvy(
      m: LabelMap): (Option[File], List[String], Boolean, Option[File]) = {
    val (ivyHome, m1) = optfile(m, "ivy-home")
    val (checksums, m2) =
      ids(m1, "checksums", BootConfiguration.DefaultChecksums)
  ...
  def getLogging(m: LabelMap): Logging =
    check("label", process(m, "level", getLevel))
  def getLevel(m: Option[String]) =
    m.map(LogLevel.apply).getOrElse(new Logging(LogLevel.Info))
  ...
  def getApplication(m: LabelMap): (Application, Value[List[String]]) = {
    ...
    val (crossVersioned, m6) =
      id(m5, "cross-versioned", CrossVersionUtil.binaryString)
        ...
        (patterns, options) match {
          ...
          case (ivy :: art :: Nil, (bo, mc, dso, cc)) =>
            Ivy(key,
                ...
                descriptorOptional = dso,
                skipConsistencyCheck = cc)
          case (Nil, (true, false, false, cc)) =>
            Maven(key, url, bootOnly = true)
  ...
  def getAppProperties(m: LabelMap): List[AppProperty] =
    for ((name, Some(value)) <- m.toList) yield {
      val map = ListMap(
        trim(value.split(",")).map(parsePropertyDefinition(name)): _*)
      AppProperty(name)(map.get("quick"), map.get("new"), map.get("fill"))
    }
  def parsePropertyDefinition(name: String)(value: String) =
    value.split("=", 2) match {
      case Array(mode, value) =>
        (mode, parsePropertyValue(name, value)(defineProperty(name)))
      case x =>
        Pre.error(
          "Invalid property definition '" + x + "' for property '" + name + "'")
    }
  def defineProperty(name: String)(action: String,
                                   requiredArg: String,
                                   optionalArg: Option[String]) =
    action match {
      case "prompt" => new PromptProperty(requiredArg, optionalArg)
      case "set"    => new SetProperty(requiredArg)
      case _ =>
        Pre.error("Unknown action '" + action + "' for property '" + name + "'")
    }
  private[this] lazy val propertyPattern = Pattern.compile(
    """(.+)\((.*)\)(?:\[(.*)\])?""") // examples: prompt(Version)[1.0] or set(1.0)
  def parsePropertyValue[T](name: String, definition: String)(
      f: (String, String, Option[String]) => T): T = {
    val m = propertyPattern.matcher(definition)
    if (!m.matches())
      Pre.error(
        "Invalid property definition '" + definition + "' for property '" + name + "'")
    val optionalArg = m.group(3)
    f(m.group(1),
      m.group(2),
      if (optionalArg eq null) None else Some(optionalArg))
  }
  ...
  def processLines(lines: List[Line]): SectionMap = {
    ...
    val s: State =
      (((ListMap.empty.default(x => ListMap.empty[String, Option[String]]),
         None): State) /: lines) {
        case (x, Comment)           => x
        case ((map, _), s: Section) => (map, Some(s.name))
        case ((_, None), l: Labeled) =>
          Pre.error("Label " + l.label + " is not in a section")
        case ((map, s @ Some(section)), l: Labeled) =>
          val sMap = map(section)
          if (sMap.contains(l.label))
            Pre.error(
              "Duplicate label '" + l.label + "' in section '" + section + "'")
...
class ParseException(val content: String,
                     val line: Int,
                     val col: Int,
                     val msg: String)
...
object ParseLine {
  def apply(content: String, line: Int) = {
    def error(col: Int, msg: String) =
      throw new ParseException(content, line, col, msg)
    def check(condition: Boolean)(col: Int, msg: String) =
      if (condition) () else error(col, msg)
    ...
    def labeled = {
      trimmed.split(":", 2) match {
        case Array(label, value) =>
          ...
          check(isNonEmpty(trimmedValue))(content.indexOf(':'),

...
final class UpdateApp(val id: Application,
                      val classifiers: List[String],
                      val tpe: String)
    extends UpdateTarget

final class UpdateConfiguration(val bootDirectory: File,
  ...
  def getScalaVersion = scalaVersion match {
    case Some(sv) => sv; case None => ""
  }
}

final class UpdateResult(val success: Boolean,
                         val scalaVersion: Option[String],
                         val appVersion: Option[String]) {
  @deprecated("Please use the other constructor providing appVersion.",
              "0.13.2")
  def this(success: Boolean, scalaVersion: Option[String]) =
    this(success, scalaVersion, None)
}
...
final class Update(config: UpdateConfiguration) {
  ...
  private def addCredentials() {
    ...
    optionProps match {
      case Some(props) =>
        extractCredentials("realm", "host", "user", "password")(props)
      case None => ()
    }
    extractCredentials("sbt.boot.realm",
                       "sbt.boot.host",
                       "sbt.boot.user",
                       "sbt.boot.password")(System.getProperties)
  }
  private def extractCredentials(keys: (String, String, String, String))(
      props: Properties) {
  ...
  private lazy val settings = {
    ...
    ivyHome match {
      case Some(dir) => settings.setDefaultIvyUserDir(dir); case None =>
    }
    addResolvers(settings)
    settings.setVariable("ivy.checksums", checksums mkString ",")
    settings.setDefaultConflictManager(
      settings.getConflictManager(ConflictManagerName))
    settings.setBaseDir(bootDirectory)
    setScalaVariable(settings, scalaVersion)
    settings
  }
  private[this] def setScalaVariable(settings: IvySettings,
                                     scalaVersion: Option[String]): Unit =
    scalaVersion match {
      case Some(sv) => settings.setVariable("scala", sv); case None =>
    }
  ...
  private lazy val ivyLockFile =
    new File(settings.getDefaultIvyUserDir, ".sbt.ivy.lock")
  ...
  private def lockedApply(target: UpdateTarget,
                          reason: String): UpdateResult = {
    ivy.pushContext()
    try { update(target, reason) } catch {
      case e: Exception =>
  ...
  private def update(target: UpdateTarget, reason: String): UpdateResult = {
    ...
    moduleID.addConfiguration(
      new IvyConfiguration(DefaultIvyConfiguration,
                           PUBLIC,
                           "",
                           new Array(0),
                           true,
    ...
    val dep = target match {
      case u: UpdateScala =>
        ...
        addDependency(moduleID,
                      scalaOrg,
                      CompilerModuleName,
                      scalaVersion,
                      "default;optional(default)",
                      u.classifiers)
        val ddesc = addDependency(moduleID,
                                  scalaOrg,
                                  LibraryModuleName,
                                  scalaVersion,
                                  "default",
        ...
        System.err.println(
          "Getting" + scalaOrgString + " Scala " + scalaVersion + " " + reason + "...")
      ...
      case u: UpdateApp =>
        ...
        val resolvedName = (app.crossVersioned, scalaVersion) match {
          ...
          case (xsbti.CrossValue.Binary, Some(sv)) =>
            app.name + "_" + CrossVersionUtil.binaryScalaVersion(sv)
          case _ => app.name
        }
        val ddesc = addDependency(moduleID,
                                  app.groupID,
                                  ...
                                  u.classifiers)
        System.err.println(
          "Getting " + app.groupID + " " + resolvedName + " " + app.getVersion + " " + reason + " (this may take some time)...")
        ddesc.getDependencyId
    }
  ...
  private def update(moduleID: DefaultModuleDescriptor,
                     target: UpdateTarget,
                     dep: ModuleId): UpdateResult = {
    ...
    val target1 = (depVersion, target) match {
      case (Some(dv), u: UpdateApp) =>
        import u._;
        new UpdateApp(id.copy(version = new Explicit(dv)), classifiers, tpe)
      case _ => target
    }
  ...
  private def addDependency(moduleID: DefaultModuleDescriptor,
                            organization: String,
                            name: String,
                            revision: String,
                            conf: String,
  ...
  private def addClassifier(dep: DefaultDependencyDescriptor,
                            name: String,
                            classifier: String) {
  ...
  private def excludeRule(organization: String, name: String): ExcludeRule = {
    val artifact =
      new ArtifactId(ModuleId.newInstance(organization, name), "*", "*", "*")
  ...
  private def resolve(eventManager: EventManager,
                      ...
                      dep: ModuleId): (Option[String], Option[String]) = {
    ...
    val resolveEngine =
      new ResolveEngine(settings, eventManager, new SortEngine(settings))
    ...
    if (resolveReport.hasError) {
      ...
      System.err.println(
        seen.toArray.mkString(System.getProperty("line.separator")))
  ...
  private[this] def extractVersion(modules: Seq[ModuleRevisionId],
                                   dep: ModuleId): Option[String] = {
    modules collectFirst {
      case m if m.getModuleId.equals(dep) => m.getRevision
    }
  }
  private[this] def moduleRevisionIDs(
      report: ResolveReport): Seq[ModuleRevisionId] = {
    import collection.JavaConverters._
    import org.apache.ivy.core.resolve.IvyNode
    report.getDependencies
      .asInstanceOf[java.util.List[IvyNode]]
      .asScala map (_.getResolvedId)
  }
  ...
  private def retrieve(eventManager: EventManager,
                       module: ModuleDescriptor,
                       target: UpdateTarget,
    ...
    val filter = (a: IArtifact) =>
      retrieveType(a.getType) && a.getExtraAttribute("classifier") == null && extraFilter(
        a)
    ...
    retrieveOptions.setDestArtifactPattern(
      baseDirectoryName(scalaOrg, scalaV) + "/" + pattern)
  ...
  private def retrieveType(tpe: String): Boolean =
    tpe == "jar" || tpe == "bundle"
  ...
  private def hasImplicitClassifier(artifact: IArtifact): Boolean = {
    ...
    artifact.getQualifiedExtraAttributes.keys
  ...
  private[this] def configureRepositoryCache(settings: IvySettings) {
    ...
    val manager =
      new DefaultRepositoryCacheManager("default-cache", settings, cacheDir) {
      ...
      case p: xsbti.PredefinedRepository =>
        ...
        p.id match {
          ...
          case ScalaToolsReleases =>
            log(
              s"$ScalaToolsReleases deprecated. use $SonatypeOSSReleases instead.")
            sonatypeReleases
          case SonatypeOSSReleases =>
            sonatypeReleases
          case ScalaToolsSnapshots =>
            log(
              s"$ScalaToolsSnapshots deprecated. use $SonatypeOSSSnapshots instead.")
  ...
  private def onDefaultRepositoryCacheManager(settings: IvySettings)(
      f: DefaultRepositoryCacheManager => Unit) {
  ...
  private def urlResolver(id: String,
                          base: String,
                          ivyPattern: String,
  ...
  private def useSecureResolvers =
  ...
  private def jcenterResolver =
    mavenResolver("JCenter", jcenterRepositoryRoot(useSecureResolvers))
  ...
  private val SnapshotPattern =
    Pattern.compile("""(\d+).(\d+).(\d+)-(\d{8})\.(\d{6})-(\d+|\+)""")
  private def scalaSnapshots(scalaVersion: String) = {
    ...
    if (m.matches) {
      ...
      mavenResolver("Sonatype Snapshots Repository",
                    "https://oss.sonatype.org/content/repositories/snapshots")
  }

  /** Logs the given message to a file and to the console. */
  private def log(msg: String) = {
    try { logWriter.println(msg) } catch {
      case e: Exception =>
        System.err.println("Error writing to update log file: " + e.toString)
    }
...
private object SbtIvyLogger {
  ...
  def acceptError(msg: String) =
    acceptMessage(msg) && !msg.startsWith(UnknownResolver)
  def acceptMessage(msg: String) =
    (msg ne null) && !msg.startsWith(IgnorePrefix)

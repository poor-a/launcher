...
object Initialize {
  ...
  def fill(file: File, spec: List[AppProperty]): Unit =
    process(file, spec, selectFill)
  ...
  def initialize(properties: Properties, name: String, init: PropertyInit) {
    init match {
      ...
      case prompt: PromptProperty =>
        ...
        readLine(
          prompt.label + prompt.default.toList
            .map(" [" + _ + "]")
            .mkString + ": ") match {
          case None => noValue
          case Some(line) =>
            val value =
              if (isEmpty(line)) orElse(prompt.default, noValue) else line

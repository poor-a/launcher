...
object Pre {
  ...
  def trimLeading(line: String) = {
    def newStart(i: Int): Int =
      if (i >= line.length || !Character.isWhitespace(line.charAt(i))) i
      else newStart(i + 1)
  ...
  def toFile(url: URL): File =
    try { new File(url.toURI) } catch {
      case _: java.net.URISyntaxException => new File(url.getPath)
    }
  ...
  final val isWindows: Boolean = System
    .getProperty("os.name")
    .toLowerCase(Locale.ENGLISH)
    .contains("windows")
  final val isCygwin: Boolean = isWindows && java.lang.Boolean
    .getBoolean("sbt.cygwin")
  ...
  private[boot] def writeProperties(properties: Properties,
                                    file: File,
                                    msg: String): Unit = {

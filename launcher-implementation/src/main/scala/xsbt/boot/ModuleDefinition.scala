...
final class ModuleDefinition(val configuration: UpdateConfiguration,
  ...
  private def fail(extra: String) =
    throw new xsbti.RetrieveException(versionString,
...
final class RetrievedModule(val fresh: Boolean,
  ...
  lazy val monkeys: Array[File] = sys.props
    .get("sbt.launcher.cp.prepend")
    .toArray
    .flatMap(ms => ms.split(",").map(new File(_)))

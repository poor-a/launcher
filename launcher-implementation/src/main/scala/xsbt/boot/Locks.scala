...
object GetLocks {
  ...
  def find: xsbti.GlobalLock =
    Loaders(getClass.getClassLoader.getParent)
      .flatMap(tryGet)
      .headOption
      .getOrElse(Locks)
  private[this] def tryGet(loader: ClassLoader): List[xsbti.GlobalLock] =
    try { getLocks0(loader) :: Nil } catch {
      case e: ClassNotFoundException => Nil
    }
...
object Locks extends xsbti.GlobalLock {
  private[this] val locks =
    new Cache[File, Unit, GlobalLock]((f, _) => new GlobalLock(f))
    ...
    val lock =
      synchronized {
        ...
        try file.createNewFile()
          ...
          case e: IOException =>
            throw new IOException(s"Failed to create lock file $file", e)
        }
  ...
  private[this] class GlobalLock(file: File) {
    ...
    def withLock[T](run: Callable[T]): T =
      synchronized {
        if (fileLocked)
          ...
          fileLocked = true
          try { ignoringDeadlockAvoided(run) } finally { fileLocked = false }
        }
      }

    // https://github.com/sbt/sbt/issues/650
    // This approach means a real deadlock won't be detected
    @tailrec private[this] def ignoringDeadlockAvoided[T](
        run: Callable[T]): T = {
      val result =
        try { Some(withFileLock(run)) } catch {
    ...
    private[this] def withFileLock[T](run: Callable[T]): T = {
      def withChannelRetries(retries: Int)(channel: FileChannel): T =
        try { withChannel(channel) } catch {
          case i: InternalLockNPE =>
            if (retries > 0) withChannelRetries(retries - 1)(channel)
            else throw i
        }

      def withChannel(channel: FileChannel) = {
        val freeLock = try { channel.tryLock } catch {
          case e: NullPointerException => throw new InternalLockNPE(e)
        }
        if (freeLock eq null) {
          System.err.println(
            "Waiting for lock on " + file + " to be available...");
          val lock = try { channel.lock } catch {
            case e: NullPointerException => throw new InternalLockNPE(e)
          }
          try { run.call } finally { lock.release() }
        } else {
          try { run.call } finally { freeLock.release() }
        }
      }
      Using(new FileOutputStream(file).getChannel)(withChannelRetries(5))
    }
  }
  private[this] final class InternalLockNPE(cause: Exception)

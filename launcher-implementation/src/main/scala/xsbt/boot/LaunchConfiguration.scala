...
final case class LaunchConfiguration(
  ...
  def withScalaVersion(newScalaVersion: String) =
    LaunchConfiguration(new Explicit(newScalaVersion),
                        ivyConfiguration,
                        app,
                        boot,
                        logging,
  ...
  def withApp(app: Application) =
    LaunchConfiguration(scalaVersion,
                        ivyConfiguration,
                        app,
                        boot,
                        logging,
  ...
  def withAppVersion(newAppVersion: String) =
    LaunchConfiguration(scalaVersion,
                        ...
                        serverConfig)
  // TODO: withExplicit
  def withVersions(newScalaVersion: String,
...
final class Implicit[T](val name: String, val default: Option[T])
    extends Value[T] {
  require(isNonEmpty(name), "Name cannot be empty")
  override def toString =
    name + (default match { case Some(d) => "[" + d + "]"; case None => "" })
}
object Value {
  ...
  def readImplied[T](s: String, name: String, default: Option[String])(
      implicit read: String => T): Value[T] =
    if (s == "read") new Implicit(name, default map read)
    else Pre.error("Expected 'read', got '" + s + "'")
}

final case class Classifiers(forScala: Value[List[String]],
                             app: Value[List[String]])
object Classifiers {
...
object LaunchCrossVersion {
  def apply(s: String): xsbti.CrossValue =
    s match {
      ...
      case x =>
        Pre.error("Unknown value '" + x + "' for property 'cross-versioned'")
    }
}

final case class Application(groupID: String,
                             name: String,
  ...
  def map(f: File => File) =
    Application(groupID,
                ...
                components,
                crossVersioned,
                classpathExtra.map(f))
}
final case class AppID(groupID: String,
                       name: String,
                       version: String,
                       mainClass: String,
...
object Application {
  ...
  private def safeCrossVersionedValue(
      id: xsbti.ApplicationID): xsbti.CrossValue =
...
object Repository {
  ...
  final case class Maven(id: String, url: URL, bootOnly: Boolean = false)
      ...
      with Repository
  final case class Ivy(id: String,
                       url: URL,
                       ivyPattern: String,
  ...
  final case class Predefined(id: xsbti.Predefined, bootOnly: Boolean = false)
      ...
      with Repository
  object Predefined {
    def apply(s: String): Predefined =
      new Predefined(xsbti.Predefined.toValue(s), false)
...
final case class BootSetup(directory: File,
                           lock: Boolean,
                           properties: File,
                           search: Search,
                           promptCreate: String,
...
final case class AppProperty(name: String)(val quick: Option[PropertyInit],
...
final class PromptProperty(val label: String, val default: Option[String])
    extends PropertyInit
...
final class Logging(level: LogLevel.Value) extends Serializable {
  ...
  private def stream(at: LogLevel.Value) =
    if (at == LogLevel.Error) System.err else System.out
}
...
final class AppConfiguration(val arguments: Array[String],

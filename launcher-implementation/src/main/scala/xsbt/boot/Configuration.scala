...
object Configuration {
  ...
  def parse(file: URL, baseDirectory: File) =
    Using(new InputStreamReader(file.openStream, "utf8"))(
      (new ConfigurationParser).apply)
  ...
  /**
    ...
    */
  @tailrec def find(args: List[String], baseDirectory: File)
    : (URL, List[String], ConfigurationStorageState.Value) =
    args match {
      case head :: tail if head.startsWith("@load:") =>
        (directConfiguration(head.substring(6), baseDirectory),
         tail,
         SerializedFile)
      case head :: tail if head.startsWith("@") =>
        (directConfiguration(head.substring(1), baseDirectory),
         tail,
         PropertiesFile)
  ...
  def setProperty(head: String) {
    head.split("=", 2) match {
      case Array("") =>
        System.err.println(s"Warning: invalid system property '$head'")
  ...
  def configurationOnClasspath: URL = {
    ...
    paths.iterator.map(getClass.getResource).find(neNull) getOrElse
      (multiPartError(
        "Could not finder sbt launch configuration.  Searched classpath for:",
        paths))
  }
  def directConfiguration(path: String, baseDirectory: File): URL = {
    try { new URL(path) } catch {
      case _: MalformedURLException =>
        configurationFromFile(path, baseDirectory)
    }
  }
  def configurationFromFile(path: String, baseDirectory: File): URL = {
    ...
    def resolve(against: URI): Option[URL] = {
      ...
      val exists = try { (new File(resolved)).exists } catch {
    ...
    if (!resolving.hasNext)
      multiPartError(
        "Could not find configuration file '" + path + "'.  Searched:",
        against)
    resolving.next()
  }
  ...
  val baseResourcePaths
    : List[String] = userConfigurationPath :: defaultConfigurationPath :: Nil

...
class Find(config: LaunchConfiguration) {
  ...
  def apply(currentDirectory: File) = {
    ...
    lazy val fromRoot =
      path(current, Nil).filter(hasProject).map(_.getCanonicalFile)
        ...
        case Search.Only =>
          if (hasProject(current))
            ...
            fromRoot match {
              ...
              case xs =>
                System.err.println(
                  "Search method is 'only' and multiple ancestor directories match:\n\t" + fromRoot
                    .mkString("\n\t"))
                System.exit(1)
                None
            }
...
object ResolvePaths {
  def apply(config: LaunchConfiguration,
            baseDirectory: File): LaunchConfiguration =

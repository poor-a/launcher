...
class ServerApplication private (provider: xsbti.AppProvider)
    extends xsbti.AppMain {
...
object ServerLocator {
  ...
  def locate(currentDirectory: File, config: LaunchConfiguration): URI =
    config.serverConfig match {
      case None =>
        sys.error("No server lock file configured.  Cannot locate server.")
      case Some(sc) =>
  ...
  def writeProperties(f: File, uri: URI): Unit = {
    ...
    Pre.writeProperties(props,
                        f,
                        s"Server Startup at ${df.format(new java.util.Date)}")
  }
...
class StreamDumper(in: java.io.BufferedReader, out: java.io.PrintStream)
    extends Thread {
  ...
  override def run(): Unit = {
    def read(): Unit =
      if (endTime.get > System.currentTimeMillis) in.readLine match {
        case null => ()
        case line =>
          out.println(line)
          out.flush()
  ...
  def close(waitForErrors: Boolean): Unit = {
    ...
    if (waitForErrors) {
      ...
      while (isAlive() && (endTime.get > System.currentTimeMillis)) Thread
        .sleep(50)
    } else {
...
object ServerLauncher {
  ...
  def launchProcessAndGetUri(cmd: List[String], cwd: File): URI = {
    ...
    try readUntilSynch(
      new java.io.BufferedReader(new java.io.InputStreamReader(stdout))) match {
      case Some(uri) => uri
      case _         =>
        ...
        sys.error(
          s"Failed to start server process in ${pb.directory} command line ${pb.command}")
    } finally {
  ...
  def javaIsAbove(currentDirectory: File, version: Int): Option[Boolean] =
    try {
      ...
      try {
        ...
        val stderr = new java.io.LineNumberReader(
          new java.io.InputStreamReader(process.getErrorStream))
        // Looking for the first line which is `java version "1.7.0_60"` or similar
        val lineOption = try Option(stderr.readLine())
        finally stderr.close()
        val pattern = java.util.regex.Pattern
          .compile("""java version "[0-9]+\.([0-9]+)\..*".*""")
        lineOption flatMap { line =>
          val matcher = pattern.matcher(line)
          if (matcher.matches()) {
            try Some(Integer.parseInt(matcher.group(1)) > version)
            catch { case NonFatal(_) => None }
          } else {
            System.err.println(
              s"Failed to parse version from 'java -version' output '$line'")
            None
          }
        }
      } finally {
      ...
      case e: IOException =>
        ...
        System.err.println(
          s"Failed to run 'java -version': ${e.getClass.getName}: ${e.getMessage}")
        None
    }

  def serverJvmArgs(currentDirectory: File,
                    serverConfig: ServerConfiguration): List[String] =
    serverJvmArgs(currentDirectory,
                  serverConfig.jvmArgs map readLines getOrElse Nil)

  final val memOptPrefixes = List("-Xmx",
                                  "-Xms",
                                  "-XX:MaxPermSize",
                                  "-XX:PermSize",
  ...
  def serverJvmArgs(currentDirectory: File,
                    baseArgs: List[String]): List[String] = {
    // ignore blank lines
    ...
    if (trimmed.exists(isMemoryOption(_)))
      ...
      val permOptions = javaIsAbove(currentDirectory, 7) match {
        case Some(true) =>
          List(s"-XX:MetaspaceSize=${defaultMinPermM}m",
               s"-XX:MaxMetaspaceSize=${defaultMaxPermM}m")
        case Some(false) =>
          List(s"-XX:PermSize=${defaultMinPermM}m",
               s"-XX:MaxPermSize=${defaultMaxPermM}m")
  ...
  def defaultLauncherLookup: File =
    try {
      ...
      fileOpt.getOrElse(
        throw new RuntimeException(
          "Could not inspect protection domain or code source"))
    } catch {
      case e: Throwable =>
        throw new RuntimeException("Unable to find sbt-launch.jar.", e)
    }
}

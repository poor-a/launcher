...
sealed class ListMap[K, V] private (backing: List[(K, V)])
  ...
  protected def copy(newBacking: List[(K, V)]): ListMap[K, V] =
    new ListMap(newBacking)
  def default(defaultF: K => V): ListMap[K, V] =
    new ListMap[K, V](backing) {
      override def apply(k: K) = super.get(k).getOrElse(defaultF(k))
      override def copy(newBacking: List[(K, V)]) =
        super.copy(newBacking).default(defaultF)
    }
...
object ListMap {
  ...
  private def remove[K, V](backing: List[(K, V)], k: K) =
    backing.filter(_._1 != k)
}
